...
struct Problem {
    ...
    cdouble coefficient(int control, double t) const {
        ...
        if (sample_count == 1) {
            ...
            const double phase = 2.0 * kPi * lo_freqs[control] * t;
            return value.real() * std::cos(phase) - value.imag() * std::sin(phase);
        }

        double position = (t - t0) / source_dt;
        if (position < 0.0) {
            position = 0.0;
        }
    ...
    void build_hamiltonian(double t) {
        std::copy(h0, h0 + static_cast<std::size_t>(n) * n, hmat.begin());
        for (int control = 0; control < control_count; ++control) {
            ...
            for (int index = 0; index < n * n; ++index) {
                hmat[index] += scale * operator_data[index];
            }
        }
    }

    void rhs(double t, const std::vector<cdouble>& state, std::vector<cdouble>& derivative) {
        ++rhs_evaluations;
        build_hamiltonian(t);
        for (int row = 0; row < n; ++row) {
            for (int batch = 0; batch < batch_count; ++batch) {
                ...
                for (int column = 0; column < n; ++column) {
                    value += hmat[row * n + column] * state[column * batch_count + batch];
                }
...
bool integrate_interval(
    ...
    std::vector<cdouble> k1(state_size), k2(state_size), k3(state_size), k4(state_size);
    std::vector<cdouble> k5(state_size), k6(state_size), k7(state_size), trial(state_size), fourth(state_size);

    double t = start;
    double step = end - start;
    double max_frequency = 0.0;
    for (int control = 0; control < problem.control_count; ++control) {
        max_frequency = std::max(max_frequency, std::abs(problem.lo_freqs[control]));
    }
    if (problem.mode == 0 && max_frequency > 0.0) {
        ...
        step = std::min(step, 0.1 / max_frequency);
    }
    ...
    while (t < end) {
        ...
        problem.rhs(t + step * (4.0 / 5.0), trial, k4);
        ...
        problem.rhs(t + step * (8.0 / 9.0), trial, k5);
...
PyObject* native_propagate(PyObject*, PyObject* args, PyObject* kwargs) {
    ...
    problem.hmat.resize(static_cast<std::size_t>(problem.n) * problem.n);
    ...
    Py_DECREF(value);
    value = PyUnicode_FromString("cpp_dopri5");
